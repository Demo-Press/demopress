...
<div class="card"><div class="kpirow"><div><div class="label">Safe release workflow</div><h2 style="margin:5px 0">Export → Validate → Activate</h2></div></div><p class="muted">Publishing now creates an untested candidate. Launch its validation demo and confirm DemoPress reaches the strict readiness signal. Only snapshots marked Validated can be activated or restored.</p></div>
<div class="card"><div class="kpirow"><div><div class="label">WordPress Agent policy</div><h2 style="margin:5px 0">Disposable demo user</h2></div>${access?'<span class="badge success">Policy detected</span>':'<span class="badge warn">Policy unavailable</span>'}</div>${access?`<div class="grid" style="margin-top:14px"><div><div class="label">Baseline role</div><div class="value">${esc(access.baseRoleName||access.baseRole)}</div></div><div><div class="label">Menu policy</div><div class="value">${access.restrictMenus?'Whitelist enabled':'Baseline permissions'}</div></div><div><div class="label">Allowed areas</div><div class="value">${access.restrictMenus?esc((access.allowedMenus||[]).join(', ')||'None selected'):'All areas permitted by role'}</div></div></div>`:`<p class="muted">The installed Agent did not report a demo-user access policy. Open WordPress → Settings → DemoPress Agent, configure Demo user access, save it and validate again.</p>`}<p class="muted" style="margin-bottom:0">The policy is stored in the golden WordPress database and travels with the snapshot.</p></div>
...
r.post("/:version/delete",(req,res)=>{try{snapshots.remove(req.params.version);res.redirect("/manage/template?saved="+encodeURIComponent("Snapshot deleted"))}catch(e){res.status(409).send(adminPage("Deletion refused",`<h1>Snapshot not deleted</h1><div class="notice">${esc(e.message)}</div><div class="actions"><a class="btn secondary" href="/manage/template">Back to Template</a><a class="btn secondary" href="/manage/presets">Review Presets</a></div>`,"template"))}});
module.exports=r;
