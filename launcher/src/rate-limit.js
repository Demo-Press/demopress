...
function ip(req){return String(req.headers["cf-connecting-ip"]||req.headers["x-forwarded-for"]||req.socket.remoteAddress||"unknown").split(",")[0].trim();}
function check(req){
 ...
 if(launches>=config.maxLaunchesPerHour)throw new Error("Hourly launch limit reached.");
 if(active>=config.maxPerIp)throw new Error("You already have the maximum number of active demos.");
 db.prepare("INSERT INTO launches(ip_address,created_at) VALUES(?,?)").run(address,now);return address;
}
