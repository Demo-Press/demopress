...
async function live(id){const d=db.prepare("SELECT * FROM demos WHERE id=?").get(id);if(!d)return null;const events=db.prepare("SELECT created_at,created_at_ms,stage,level,message FROM provisioning_events WHERE demo_id=? ORDER BY id").all(id);let wpLogs=d.archived_wp_logs||"",dbLogs=d.archived_db_logs||"";if(d.status!=="deleted"&&d.container_id)try{wpLogs=await actions.logs(docker.getContainer(d.container_id),250)}catch(_){}if(d.status!=="deleted"&&d.db_container_id)try{dbLogs=await actions.logs(docker.getContainer(d.db_container_id),250)}catch(_){}return{demo:d,events,wpLogs,dbLogs}}
function diag(x){const d=x.demo,start=(d.provision_started_at||d.created_at)*1000,end=d.provision_finished_at?d.provision_finished_at*1000:Date.now(),l=["DEMOPRESS DEMO DIAGNOSTIC","=========================",`Platform: ${d.platform_version||config.platformVersion}`,`Generated: ${new Date().toISOString()}`,`Demo: ${d.id}`,`Type: ${d.demo_type||"public"}`,`Status: ${d.status}`,`Stage: ${d.provision_stage}`,`Message: ${d.status_message||""}`,d.deleted_at?`Deleted: ${new Date(d.deleted_at*1000).toISOString()}`:null,d.delete_reason?`Delete reason: ${d.delete_reason}`:null,`Template: ${d.template_version}`,`Elapsed: ${((end-start)/1000).toFixed(3)}s`,"","STAGE TIMINGS",`database_ms=${d.database_ms||0}`,`import_ms=${d.import_ms||0}`,`wordpress_ms=${d.wordpress_ms||0}`,`finalise_ms=${d.finalise_ms||0}`,`routing_ms=${d.routing_ms||0}`,"","EVENT TIMELINE"];for(const e of x.events){const ms=e.created_at_ms||e.created_at*1000;l.push(`${new Date(ms).toISOString()} +${((ms-start)/1000).toFixed(3)}s [${e.level}] [${e.stage}] ${e.message}`)}l.push("","WORDPRESS LOGS",x.wpLogs||"(none)","","MARIADB LOGS",x.dbLogs||"(none)");return l.filter(Boolean).join("\n")}
